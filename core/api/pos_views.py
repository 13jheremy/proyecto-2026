...
def crear_venta_pos(request):
    ...
    try:
        with transaction.atomic():
            ...
            impuesto_porcentaje = Decimal(str(request.data.get('impuesto_porcentaje', 0)))
            ...
            for item in productos:
                ...
                try:
                    producto = Producto.objects.get(id=producto_id, eliminado=False, activo=True)
            ...
            logger.info(f"Venta POS creada #{venta.id} por usuario {request.user.username}")
...
def registrar_pago_venta(request):
    ...
    try:
        ...
        logger.info(f"Pago registrado #{pago.id} para venta #{venta.id} por usuario {request.user.username}")
        return Response(response_data, status=status.HTTP_201_CREATED)
...
def buscar_tecnicos_pos(request):
    ...
    try:
        ...
        tecnicos = Usuario.objects.select_related('persona_asociada').prefetch_related('roles__rol').filter(
        ...
        for tecnico in tecnicos:
            results.append({
                ...
                'nombre_completo': tecnico.persona_asociada.nombre_completo if tecnico.persona_asociada else tecnico.correo_electronico,
                'persona_asociada': {
                ...
                } if tecnico.persona_asociada else None,
...
def crear_mantenimiento_pos(request):
    ...
    try:
        ...
        user_roles = request.user.roles.filter(activo=True).values_list('rol__nombre', flat=True)
        ...
        with transaction.atomic():
            ...
            if fecha_ingreso:
                try:
                    ...
                    fecha_ingreso_parsed = datetime.fromisoformat(fecha_ingreso.replace('Z', '+00:00'))
                except:
            ...
            if fecha_entrega:
                try:
                    fecha_entrega_parsed = datetime.fromisoformat(fecha_entrega.replace('Z', '+00:00'))
                except:
                    fecha_entrega_parsed = None
            ...
            for servicio_data in servicios:
                ...
                try:
                    servicio = Servicio.objects.get(id=servicio_id, eliminado=False, activo=True)
            ...
            for recordatorio_data in recordatorios_data:
                ...
                if categoria_servicio_id and fecha_programada:
                    try:
                        categoria_servicio = CategoriaServicio.objects.get(id=categoria_servicio_id, eliminado=False, activo=True)
                        
                        # Parsear fecha programada
                        from datetime import datetime
                        fecha_programada_parsed = datetime.fromisoformat(fecha_programada).date()
            ...
            logger.info(f"Mantenimiento POS creado #{mantenimiento.id} por usuario {request.user.username}")
            return Response(response_data, status=status.HTTP_201_CREATED)
...
def buscar_motos_pos(request):
    ...
    try:
        ...
        for m in all_motos:
            logger.info(f"  - ID: {m.id}, Placa: {m.placa}, Marca: {m.marca}, Activo: {m.activo}, Eliminado: {m.eliminado}")
        ...
        logger.info(f"🔍 MOTO SEARCH DEBUG - Motos encontradas con filtros: {motos.count()}")
        
        results = []
        for moto in motos:
            logger.info(f"🔍 MOTO SEARCH DEBUG - Procesando moto: {moto.placa} - {moto.marca}")
            logger.info(f"🔍 MOTO SEARCH DEBUG - Propietario: {moto.propietario}")
            logger.info(f"🔍 MOTO SEARCH DEBUG - Propietario nombre: {getattr(moto.propietario, 'nombre_completo', 'NO_NOMBRE')}")
            ...
            if moto.propietario:
                ...
                logger.info(f"🔍 MOTO SEARCH DEBUG - Propietario data: {propietario_data}")
...
def buscar_servicios_pos(request):
    ...
    try:
        ...
        servicios_activos = Servicio.objects.filter(eliminado=False, activo=True).count()
        servicios_inactivos = Servicio.objects.filter(eliminado=False, activo=False).count()
        servicios_eliminados = Servicio.objects.filter(eliminado=True).count()

        logger.info(f"🔍 SERVICIOS POS DEBUG - Total servicios en DB: {total_servicios}")
        logger.info(f"🔍 SERVICIOS POS DEBUG - Servicios activos: {servicios_activos}")
        logger.info(f"🔍 SERVICIOS POS DEBUG - Servicios inactivos: {servicios_inactivos}")
        logger.info(f"🔍 SERVICIOS POS DEBUG - Servicios eliminados: {servicios_eliminados}")

        if not query and not categoria_id:
            logger.info("🔍 SERVICIOS POS DEBUG - Sin query ni categoria, retornando lista vacía")
            return Response({
                'success': True,
                'data': [],
                'status': 200,
        ...
        servicios = Servicio.objects.filter(filters).select_related('categoria_servicio')[:20]

        logger.info(f"🔍 SERVICIOS POS DEBUG - Servicios encontrados con filtros: {servicios.count()}")

        # Log de los primeros servicios encontrados
        for i, servicio in enumerate(servicios[:3]):
            logger.info(f"🔍 SERVICIOS POS DEBUG - Servicio {i+1}: ID={servicio.id}, Nombre='{servicio.nombre}', Activo={servicio.activo}, Eliminado={servicio.eliminado}")
        ...
        for servicio in servicios:
            results.append({
                ...
                'categoria': {
                    'id': servicio.categoria_servicio.id if servicio.categoria_servicio else None,
                    'nombre': servicio.categoria_servicio.nombre if servicio.categoria_servicio else 'Sin categoría'
                },
                'display_text': f"{servicio.nombre} - ${servicio.precio}"
            })

        logger.info(f"🔍 SERVICIOS POS DEBUG - Resultados finales: {len(results)} servicios")
...
def ajustar_inventario(request):
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(f"Ajuste de inventario: {inventario.producto.nombre} de {stock_anterior} a {nuevo_stock}")
            return Response(response_data)
...
def dashboard_pos_stats(request):
    ...
    try:
        ...
        response_data = {
            'ventas_hoy': {
                'total': str(ventas_hoy['total_ventas'] or Decimal('0.00')),
